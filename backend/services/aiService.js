...
async function generateProfileBio(user, profile, hints = {}) {
  ...
  const system = `You write short football profile bios for XTalenti (talent platform).
...
async function generateScoutSummary(targetUser, targetProfile, scoutUser) {
  ...
  const system = `You are a football scout assistant for XTalenti.
...
async function suggestPostCaption(hints = {}) {
  const system = `You suggest short social post captions for football players on XTalenti.
